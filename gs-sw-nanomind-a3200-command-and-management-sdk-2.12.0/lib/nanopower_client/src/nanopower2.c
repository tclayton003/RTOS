...
int eps_hk_get(eps_hk_t * hk) {
	int status = eps_hk_get_p(hk, eps_node, EPS_PORT_HK, eps_timeout);
	return status;
}

int eps_hk_get_p(eps_hk_t * hk, uint8_t dest, uint8_t port, uint32_t timeout) {
	uint8_t req = 0;
	int status = csp_transaction(CSP_PRIO_HIGH, dest, port, timeout, &req, 2, hk, sizeof(eps_hk_t));
