...
static gs_sem_t sem_fs_mounted = NULL;
...
{
    GS_A3200_TELEMETRY_FS_MOUNTED_SET_NO_CALLBACK((result == GS_OK) ? true : false);
    if (sem_fs_mounted) {
        gs_sem_post(sem_fs_mounted);
    }
}
...
static void init_csp()
{
    ...
    if (GS_A3200_BOARD_KISS_EN_GET_NC()) {
        gs_csp_kiss_init2(GS_A3200_BOARD_KISS_UART_GET_NC(), 0, NULL, false, NULL);
    }
    ...
    if (GS_A3200_BOARD_CAN_EN_GET_NC()) {
        gs_can_init(0, GS_A3200_BOARD_CAN_BRATE_GET_NC());
...
void * gs_a3200_sdk_init_task(void * param)
{
    ...
    // Initialize CSP and Start the router task
    init_csp();
    ...
    gs_sem_create(0, &sem_fs_mounted); 
    ...
    if (sem_fs_mounted) {
        gs_sem_wait(sem_fs_mounted, 300000);
    }
