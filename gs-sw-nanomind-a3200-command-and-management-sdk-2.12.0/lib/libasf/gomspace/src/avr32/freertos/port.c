...
/* Functions below is called directly from the OS scheduler, that is written with no
   optimizations in mind. Therefore we disable optimization for the rest of the file.
   Do not remove the pragma below */
...
static void prvSetupTimerInterrupt(void)
{
	...
	.tcclks   = TC_CLOCK_SOURCE_TC4                /* Internal source clock 2. */
	};
	...
	#if( configTICK_USE_TC==1 )
	{
		...
		tc_write_rc( tc, configTICK_TC_CHANNEL, (unsigned short) ((sysclk_get_peripheral_bus_hz(&AVR32_TC0) / 32) / configTICK_RATE_HZ ));
