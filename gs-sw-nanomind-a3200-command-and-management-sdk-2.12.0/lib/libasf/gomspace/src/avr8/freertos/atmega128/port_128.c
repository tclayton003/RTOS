...
/* 0x10 can not be used to set the interrupt enable bit on the AT90can128 MCU */
...
portSTACK_TYPE *pxPortInitialiseStack(portSTACK_TYPE *pxTopOfStack,
	...
	*pxTopOfStack
			= (portSTACK_TYPE ) (usAddress & (unsigned portSHORT ) 0x00ff);
	pxTopOfStack--;
...
static void prvSetupTimerInterrupt(uint32_t fcpu) {
	...
	OCR3A = (uint16_t) (fcpu / (configTICK_RATE_HZ * portCLOCK_PRESCALER)) - 1;

	/* Setup clock source and compare match behaviour. */
	TCCR3B = portCLEAR_COUNTER_ON_MATCH | portPRESCALE_64;
	...
	TIMSK3 |= 1;
#else
	ETIMSK |= portCOMPARE_MATCH_A_INTERRUPT_ENABLE;
#endif
}
