...
gs_error_t gs_hyp_st200_init(const uint8_t device, gs_hyp_st200_handle_t * handle)
{
    ...
    error = gs_hyp_st200_send_command(*handle, "ping\r", true, false);
    if (error)
    {
        return error;
    }

    error = gs_hyp_st200_send_command(*handle, "set dumping-mode polling\r", false, false);
    if (error)
    {
        return error;
    }

    error = gs_hyp_st200_send_command(*handle, "set keep-alive off\r", false, false);
    if (error)
    {
        return error;
    }
    ...
    return gs_hyp_st200_send_command(*handle, "set frame-type binary standard\r", false, false);
}
...
gs_error_t gs_hyp_st200_send_command(const gs_hyp_st200_handle_t handle, const char* command, bool no_response, bool print_response)
{
    ...
    {
        gs_hexdump(buffer, 600);
    }

    for (size_t search=0; (search < (end_idx - 5)) && (search < 600); search++)
    {
...
{
    gs_error_t error = gs_hyp_st200_send_command(handle, "show record\r", true, false);
    if (error)
    {
        return error;
    }
    ...
    error = gs_hyp_st200_crc(destuff, (size_t)53);
    if (error)
    {
        return error;
    }    

    return gs_hyp_st200_parse_response(destuff, out_len, sample);
}
