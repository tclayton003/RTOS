...
// Default timeout (trying to compensate for slower code execution when running from RAM image)
static uint16_t SPI_CS_TIMEOUT_TICKS = 400 / portTICK_RATE_MS;
...
gs_error_t gs_spi_setup_chip(gs_spi_chip_t * chip)
{
...
gs_error_t gs_spi_write(gs_spi_chip_t * chip, uint16_t data)
{
    unsigned int timeout = SPI_TIMEOUT;
    volatile avr32_spi_t *spi = spidev[chip->handle].base;
...
gs_error_t gs_spi_read(gs_spi_chip_t * chip, uint16_t * data)
{
    unsigned int timeout = SPI_TIMEOUT;
    volatile avr32_spi_t *spi = spidev[chip->handle].base;

    /* Poll for data ready or tx empty */
...
gs_error_t gs_spi_select_chip(gs_spi_chip_t * chip)
{
    GS_CHECK_HANDLE(chip != NULL);
    ...
    /* Lock spi device */
    if (xSemaphoreTake(spidev[chip->handle].lock, SPI_CS_TIMEOUT_TICKS) == pdFALSE) {
...
gs_error_t gs_spi_unselect_chip(gs_spi_chip_t * chip)
{
    /**
     * Unselect chip:
     * The following code is a snippet from the atmel asf SPI driver
...
gs_error_t gs_spi_master_transactions(uint8_t slave, gs_spi_master_trans_t *trans, size_t count, int timeout_ms)
{
    ...
    gs_error_t ret = gs_spi_select_chip(&pslave->chip);
    if (ret == GS_OK)
    {
...
{
    SPI_CS_TIMEOUT_TICKS = milisec / portTICK_RATE_MS;
    return GS_OK;
}
