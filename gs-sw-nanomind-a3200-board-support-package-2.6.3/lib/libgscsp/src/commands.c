...
static int cmd_uptime(gs_command_context_t *ctx)
{
    ...
    if (res == GS_OK) {
        uint32_t uptime;
        res = gs_csp_error(csp_get_uptime(node, timeout, &uptime));
        if (res == GS_OK) {
            char buf[50];
...
static int cmd_cmp_route_set(gs_command_context_t *ctx)
{
    ...
    fprintf(ctx->out, "Sending route_set to node %"PRIu8" timeout %"PRIu32"\r\n", node, timeout);
...
static int cmd_cmp_ifc(gs_command_context_t *ctx)
{
    ...
    uint8_t node = atoi(ctx->argv[1]);
    const char * interface = ctx->argv[2];
    ...
    fprintf(ctx->out, "Requesting interface stats for interface %s\r\n", interface);
...
static int cmd_cmp_peek(gs_command_context_t *ctx)
{
    ...
    fprintf(ctx->out, "Dumping mem from node %u addr 0x%"PRIx32" len %"PRIx32" timeout %"PRIu32"\r\n", node, addr, len, timeout);
...
static int cmd_cmp_poke(gs_command_context_t *ctx)
{
    ...
    if (len > CSP_CMP_PEEK_MAX_LEN) {
        fprintf(ctx->out, "Max length is: %u\r\n", CSP_CMP_PEEK_MAX_LEN);
        return GS_ERROR_RANGE;
    }

    fprintf(ctx->out, "Writing to mem at node %u addr 0x%"PRIx32" len %"PRIx32" timeout %"PRIu32"\r\n", node, addr, len, timeout);
...
static const gs_command_t GS_COMMAND_ROOT csp_commands[] = {
    {
        .name = "ping",
        .help = "csp: Ping",
        .usage = "[node] [timeout] [size] [opt: r|x|h|c] [count]",
        .handler = cmd_ping,
    },{
        .name = "rps",
        .help = "csp: Remote ps",
